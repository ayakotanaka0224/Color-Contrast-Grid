...
const getContrast = (fillColor, rgbColor) => {
    const l1 = getRelativeLuminance(fillColor["r"], fillColor["g"], fillColor["b"]);
...
const createFigmaComponent = (props, size) => {
...
const createFigmaFrame = (props, size) => {
...
const createFigmaText = (content, props) => {
...
figma.ui.onmessage = (msg) => {
    ...
    if (msg.type === "create-rectangles") {
        ...
        const gridColorStyles = localColorStyles.filter((item) => item.paints[0].type == "SOLID");
        createColorNameTile();
        ...
        gridColorStyles.map((_, index) => tableContainer.appendChild(tableColumn(gridColorStyles, index, msg.array)));
