...
function tableHeader(colorStyles) {
  ...
  colorStyles.map((paintStyle) => {
    ...
    instance.children.map(
      (child) =>
        (child.fills = [{ type: "SOLID", color: { r: rgb, g: rgb, b: rgb } }])
    );
...
function tableColumn(colorStyles, index, array) {
  ...
  let rgb = 1;
  const rgbRelativeLuminance = getRelativeLuminance(
    rgbColor.r,
    rgbColor.g,
    rgbColor.b
  );
  if (rgbRelativeLuminance > 0.5) {
    rgb = 0;
  }
  instance.children[0].fills = [
    { type: "SOLID", color: { r: rgb, g: rgb, b: rgb } },
  ];
  ...
  colorStyles.map((paintStyle, index) => {
    ...
    let fillRgb = 1;
    const fillRelativeLuminance = getRelativeLuminance(
      fillColor.r,
      fillColor.g,
      fillColor.b
    );
    if (fillRelativeLuminance > 0.5) {
      fillRgb = 0;
    }
    ...
    badgeFrame[1].fills = [
      { type: "SOLID", color: { r: fillRgb, g: fillRgb, b: fillRgb } },
    ];
